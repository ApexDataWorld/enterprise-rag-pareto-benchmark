...
def run_benchmark(config: dict[str, Any], output_dir_override: str | None = None) -> dict[str, Path]:
    ...
    for variant in config["variants"]:
        retriever = Retriever(chunks, variant["retriever"])
        variant_rows = []
    ...
    rerank_rows = [row for row in plot_rows if row["variant"] in {"lexical_k5", "lexical_k5_rerank", "dense_k5", "dense_k5_rerank"}]
    ...
    qt_plot_rows = [
        ...
        if row["variant"] == "hybrid_dense_k5_rerank"
    ]
...
def _write_final_summary(path: Path, rows: list[dict[str, Any]], stats_rows: list[dict[str, Any]], config: dict[str, Any]) -> None:
    lines = [
        ...
        f"- Dense retrieval provider: {config.get('dense_retrieval', {}).get('provider', 'not_configured')}",
    ...
    dense_stats = [
        ...
        if row["candidate"] in {"dense_k5", "dense_k5_rerank", "hybrid_dense_k5_rerank"}
...
def _write_consistency_notes(path: Path, rows: list[dict[str, Any]], config: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "The uploaded PDF should be treated as stale if it still says six configurations or lists dense retrieval as future work.",
        "",
        ...
        "- Dense retrieval should not be described as future work in V3.",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
